...
void platform_file_directory_get_files(const c_file_path& directory, c_stack<c_file_path> const_ptr out_files)
{
	t_string_128 wildcard("\\*");
	t_string_256 directory_no_wildcard(directory.get_full_path());
	t_string_256 directory_with_wildcard(directory.get_full_path());

	if (directory.ends_with(wildcard))
	{
		directory_no_wildcard.pop();
		directory_no_wildcard.pop();
	}
	else
	{
		directory_with_wildcard.append(wildcard.get_const_char());
	}

	WIN32_FIND_DATA file_data;
	HANDLE find_handle = INVALID_HANDLE_VALUE;

	find_handle = FindFirstFileA(directory_with_wildcard.get_const_char(), &file_data);

	if (find_handle != INVALID_HANDLE_VALUE)
	{
		do
		{
			c_file_path file(file_data.cFileName);
			out_files->push() = c_file_path::combine(directory_no_wildcard, file);
			NOP();
		} while (FindNextFileA(find_handle, &file_data) != 0);
	}
}
